      ...
      columns: [
        ...
        },
        {
          ...
          render: (text, record, dataIndex) => {
            return (
              <div>
                {record.serviceInfo.length === 0 ? (
                  <span>当前用户未完善基本信息</span>
                ) : (
                  <Switch
                    disabled={true}
                    checkedChildren="已预约"
                    ...
                    defaultChecked={text}
                    onChange={() => this.onChange(text, record._id, record)}
                    key={index}
                  />
                )}
              </div>
          ...
          render: (text, record, dataIndex) => {
            return (
              <Button
                ...
                onClick={() => this.handle_Sq_FW(record._id)}
              >
                立即申请
              </Button>
            );
        ...
        },
        {
          ...
          //     <div>
          //       <Tooltip title="详情">
          //         <DiffTwoTone onClick={() => this.handleRecord(record)} />
          //       </Tooltip>
          //       <Tooltip title="查看评价">
          //         <MessageTwoTone onClick={() => this.showModal(record)} />
          //       </Tooltip>
          //     </div>
          //   ),
        },
          ...
          >
            <Card bordered={false} style={{ width: 350 }}>
              {this.state.pingjia.map((item, index) => (
                <p>
                  内容:{item.value}。来自用户：{item.username}的评价 时间:{item.pjtime}{' '}
                </p>
              ))}
