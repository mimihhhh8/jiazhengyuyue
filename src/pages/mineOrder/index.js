...
class index extends Component {
          ...
          render: (text, record) => {
            ...
            return (
              <div>
                {text === undefined ? (
                  <Button
                    onClick={() => this.showModal(record._id, record, record.char, record.username)}
                  >
                    结束预约
                  </Button>
                ) : (
  ...
  showModal = (record, s, char, username) => {
  ...
  cancleOrder(workerId) {
    let data = {
      ...
      pjtime: moment().format('YYYY-MM-DD HH:mm'),
      fwname: this.state.name,
    ...
    return (
      <div>
        ...
        >
          ...
          <p>
            {' '}
            应结算费用:<span style={{ color: 'red' }}>{this.state.hours * this.state.char}￥</span>
            (不满一小时按一小时计算)
          </p>
          <p>评价：</p>
          <TextArea
  ...
  getOrderDetail() {
    ...
    new Promise((resolve, reject) => {
      ...
      this.setState({
        list: res.data.data.orderinfo.splice(1.1),
      });
    });
  }
